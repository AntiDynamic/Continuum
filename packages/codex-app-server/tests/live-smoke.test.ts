...
describe("installed Codex App Server live smoke",()=>{
  it("runs a genuine isolated shadow turn",async(context)=>{
    ...
    try{
      ...
      const result=await new CodexExecutionService().runShadow({cwd:repo,task:"Fix src/add.ts so add(1, 2) returns 3. Do not modify unrelated.txt.",mode:"shadow",...(model?{model}:{}),sandbox:"workspace-write",approvalPolicy:"never",timeoutMs:300000});
      ...
      expect(result.report.execution.codexVersion).not.toBe("fixture");expect(result.authenticationMode).not.toBe("none");expect(await readFile(join(repo,"src","add.ts"),"utf8")).toContain("a + b");expect(await readFile(join(repo,"unrelated.txt"),"utf8")).toBe("do not change\n");
