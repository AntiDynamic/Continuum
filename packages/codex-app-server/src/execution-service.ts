...
export async function openCodexDatabase(cwd:string,repository?:string):Promise<{db:Db;root:string}>{
  const root=await getRepositoryRoot(resolve(cwd,repository??"."));const dbPath=join(root,".continuum","continuum.db");
  if(!existsSync(dbPath))throw new Error("Continuum is not initialized. Run 'continuum init' and 'continuum index' first.");
  const db=openDatabase(dbPath);migrate(db);return{db,root};
}
...
export class CodexExecutionService {
  async runShadow(options:CodexShadowOptions):Promise<CodexShadowResult>{
    ...
    try{
      ...
      const recordRaw=async(message:CodexRawMessage):Promise<void>=>{
        const identity=ids(message);const normalized=message.direction==="server_to_client"||message.direction==="server_stderr"?normalizeCodexMessage(message):{normalized:[],threadId:identity.threadId,turnId:identity.turnId,itemId:identity.itemId};
        executions.recordReceived({raw:{execution_id:executionId,direction:message.direction,message_category:message.category,method:message.method,request_id:message.requestId===null?null:String(message.requestId),thread_id:identity.threadId,turn_id:identity.turnId,item_id:identity.itemId,timestamp:message.timestamp,raw_json:message.raw},normalized:normalized.normalized,...(normalized.usage?{usage:normalized.usage}:{}),...(normalized.diff?{diff:normalized.diff}:{})});
      };
      const approval=async(request:CodexServerRequestContext):Promise<unknown>=>{
        const supported=request.method==="item/commandExecution/requestApproval"||request.method==="item/fileChange/requestApproval";
        if(!supported)throw new CodexIntegrationError("UNSUPPORTED_SERVER_REQUEST",`Unsupported Codex server request: ${request.method}`);
        const decision=options.approvalHandler?await options.approvalHandler(request):"decline";
        executions.recordReceived({raw:{execution_id:executionId,direction:"client_to_server",message_category:"notification",method:"continuum/approvalDecision",request_id:String(request.id),thread_id:isRecord(request.params)?value(request.params,"threadId"):null,turn_id:isRecord(request.params)?value(request.params,"turnId"):null,item_id:isRecord(request.params)?value(request.params,"itemId"):null,timestamp:new Date().toISOString(),raw_json:JSON.stringify({decision})},normalized:[{eventType:"approval_decision",evidenceType:"directly observed",confidence:"high",payload:{requestMethod:request.method,decision}}]});
        return{decision};
      };
      client=new StdioCodexAppServerClient();
      await client.start({...options.process,...(options.process?.executable?{executable}:{}),cwd:opened.root,onRawMessage:recordRaw,onServerRequest:approval,onFatalError:(error)=>completionReject(error),onNotification:async(method,params)=>{
        ...
        }else if(method==="process/exited"&&!(isRecord(params)&&params.expected===true))completionReject(new CodexIntegrationError("UNEXPECTED_PROCESS_EXIT","Codex App Server exited before turn completion."));
      }});
  ...
  async report(cwd:string,executionId:string,repository?:string):Promise<ShadowFlightRecorderReport>{const opened=await openCodexDatabase(cwd,repository);try{return buildShadowReport(opened.db,executionId,opened.root);}finally{opened.db.close();}}
  async status(cwd:string,executionId:string,repository?:string):Promise<unknown>{const opened=await openCodexDatabase(cwd,repository);try{return new CodexExecutionRepository(opened.db).findRequired(executionId);}finally{opened.db.close();}}
  async list(cwd:string,repository?:string,limit=20):Promise<unknown>{const opened=await openCodexDatabase(cwd,repository);try{return new CodexExecutionRepository(opened.db).list(undefined,limit);}finally{opened.db.close();}}
}
