...
const filePath = path.resolve('packages/gemini-adapter/src/gemini-adapter.ts');
...
content = content.replace(/async\s+\*run\(input:\s+AgentRunInput\):\s+AsyncIterable<AgentEvent>\s+\{[\s\S]*?async\s+cancel\(runId:\s+string\):\s+Promise<void>\s+\{/, newRunMethod);

fs.writeFileSync(filePath, content);
console.log("Successfully replaced run method.");
