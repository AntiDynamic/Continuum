...
describe("Phase 3C standalone acceptance",()=>{
 ...
 it("survives fresh processes and reconstructs all evidence",async()=>{
  ...
  const handoff=JSON.parse((await continuum(repo,"session","handoff",sessionId,"--json")).stdout);expect(handoff.schemaVersion).toBe("continuum.context-preflight-handoff.v1");expect(handoff.prompt).toContain("CONTINUUM PREFLIGHT CONTEXT v1");expect(handoff.prompt).toContain(sessionId);
