...
export async function runSessionPlan(id:string,options:SessionBaseOptions):Promise<void>{const result=await use(options,s=>s.plan(id));if(options.json)return output(result,true);line(bold("CONTINUUM CONTEXT PLAN"));blank();kv("Session",result.sessionId);kv("Task",result.task);kv("Risk",result.riskLevel);kv("Budget",fmt(result.budget.deliveredEstimatedTokens)+" / "+fmt(result.budget.maximumEstimatedTokens));section("Coverage");kv("Required",result.requiredCoverage.join(", ")||"none");kv("Remaining",result.remainingCoverage.join(", ")||"none");section("Next actions");for(const action of result.nextActions)line("  "+action);}
export async function runSessionRecover(id:string,options:SessionBaseOptions):Promise<void>{const result=await use(options,s=>s.recover(id));if(options.json)return output(result,true);line(bold("CONTINUUM CONTEXT RECOVERY"));blank();kv("Session",result.sessionId);kv("Status",result.status);kv("Task",result.task);kv("Active context",String(result.progress.activeContextItemCount));kv("Coverage remaining",result.coverageRemaining.join(", ")||"none");kv("Next action",result.nextAction);if(result.blockers.length){section("Blockers");for(const blocker of result.blockers)line("  "+blocker);}}
